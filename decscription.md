...
<details>
<summary>where use</summary>
По сути, легковес — это «метод нормализации объектов», в котором общие свойства выделяются в общие объекты легковеса. (Примечание: идея похожа на нормализацию модели данных, процесс, в котором разработчик модели пытается минимизировать избыточность).

Пример шаблона Flyweight находится в самом движке JavaScript, который поддерживает список неизменяемых строк, которые являются общими для всего приложения.

Другие примеры включают символы и стили строк в текстовом процессоре или «цифровые приемники» в приложении телефонной сети общего пользования. Вы найдете легковесы в основном в служебных приложениях, таких как текстовые процессоры, графические программы и сетевые приложения; они реже используются в бизнес-приложениях, управляемых данными.
...
<details>
<summary>comment</summary>
В нашем примере кода мы создаем компьютеры. Многие модели, производители и комбинации процессоров являются общими, поэтому эти характеристики вынесены за скобки и являются общими для объектов-приспособленцев.

Поддерживает FlyweightFactory пул Flyweightобъектов. При запросе Flyweightобъекта FlyweightFactory проверяется, существует ли он уже; если нет, новый будет создан и сохранен для дальнейшего использования. Все последующие запросы к тому же компьютеру будут возвращать сохраненный Flyweightобъект.

Несколько разных компьютеров добавляются в файл ComputerCollection. В конце у нас есть список из 7 Computerобъектов, которые разделяют только 2 Flyweights. Это небольшая экономия, но при больших наборах данных экономия памяти может быть значительной.
</details>
...
<details>
<summary>where use</summary>
В объектно-ориентированном программировании объекты выполняют ту работу, которую объявляют через свой интерфейс (свойства и методы). Клиенты этих объектов ожидают, что эта работа будет выполнена быстро и качественно. Однако бывают ситуации, когда объект сильно ограничен и не может соответствовать своим обязанностям. Обычно это происходит, когда существует зависимость от удаленного ресурса (что приводит к задержке в сети) или когда загрузка объекта занимает много времени.

В подобных ситуациях вы применяете шаблон прокси и создаете прокси-объект, который «заменяет» исходный объект. Прокси перенаправляет запрос целевому объекту. Интерфейс объекта Proxy такой же, как у исходного объекта, и клиенты могут даже не знать, что имеют дело с прокси, а не с реальным объектом.
...
<details>
<summary>comment</summary>
Объект GeoCoderимитирует службу геокодирования Google Maps. В геокодировании вы указываете местоположение (место на земле), и оно возвращает его широту/долготу (latlng). Мы GeoCoderможем разрешить только 4 локации, а на самом деле их миллионы, потому что это касается стран, городов и улиц.

Программист решил реализовать прокси-объект, потому что GeoCoderон относительно медленный. Прокси-объект называется GeoProxy. Известно, что поступает много повторных запросов (для одного и того же местоположения). Чтобы ускорить работу, GeoProxyкэширует часто запрашиваемые местоположения. Если местоположение еще не кэшировано, оно обращается к реальной GeoCoderслужбе и сохраняет результаты в кеше.

Запрашиваются местоположения нескольких городов, и многие из них относятся к одному и тому же городу. GeoProxyсоздает свой кеш, поддерживая эти вызовы. В итоге GeoProxy<обработал 11 запросов, но выйти пришлось GeoCoderвсего 3 раза. Обратите внимание, что клиентская программа ничего не знает о прокси-объекте (она вызывает тот же интерфейс со стандартным методом getLatLng).
</details>
